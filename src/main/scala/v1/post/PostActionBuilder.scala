...
class PostBaseController @Inject()(pcc: PostControllerComponents) extends BaseController with RequestMarkerContext {
  ...
  def verifyParameter(name: String, request: PostRequest[AnyContent]): Boolean = {
    request.getQueryString(name) match {
      case None => true
      case Some(str) => Try{str.toInt}.isSuccess
    }
  }

  def verifyIntParameter(name: String, request: PostRequest[AnyContent]): Boolean = {
    request.getQueryString(name) match {
      case None => true
      case Some(str) => Try{str.toInt}.isSuccess
    }
  ...
  def verifyStringParameter(name: String, request: PostRequest[AnyContent]): Boolean = {
    request.getQueryString(name) match {
      case None => true
      case Some(str) => str.length > 0 && !str.startsWith("?")
    }
  }
  def verifyGenderParameter(name: String, request: PostRequest[AnyContent]): Boolean = {
    request.getQueryString(name) match {
      case None => true
      case Some(str) => str=="m" || str == "f"
    }
  }
}